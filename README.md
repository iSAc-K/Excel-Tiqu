...
- 当正式 Excel 文件名未命中日期或品类时，会再用外层压缩包名或文件夹名识别一次。例如外层名称包含 `1216` 和 `银翅膀图片项链` 时，会写入日期“12月16日”和“翅膀图片项链”工作表。
...
- 压缩包文件名只要包含“修改”，会在解压前直接跳过，并把来源压缩包复制到输入目录下的 `未处理压缩包` 文件夹；Dry Run 只预览，不实际复制。
- 正式 Excel 文件名前两个字是“修改”时也会跳过不处理，并把来源压缩包复制到输入目录下的 `未处理压缩包` 文件夹。
- 日期和品类识别增加外层名称兜底：正式 Excel 文件名识别不到日期，或品类识别为“未分类”时，再尝试使用外层压缩包名或文件夹名识别。
